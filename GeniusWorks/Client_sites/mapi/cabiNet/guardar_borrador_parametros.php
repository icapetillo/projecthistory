...
$sql = "SELECT * FROM semp_borradores_parametros WHERE mes=$mes AND ano=$ano AND idalumno=$id_alumno AND idpar=$param";
...
//Borrar registros existentes
	mysql_query("DELETE FROM semp_borradores_parametros WHERE idalumno=$id_alumno AND mes=$mes AND ano=$ano AND idpar=$param") or die(mysql_error());
	$sql = "SELECT * FROM semp_parametros_desarrollo WHERE idparametro=$param";
	$result = mysql_query($sql) or die(mysql_error());
	$num = mysql_num_rows($result);
	//Insertar nuevo registro
	for ($k=0;$k<$num;$k++){
		mysql_query("INSERT INTO semp_borradores_parametros (idalumno, mes, ano, idpar, idparametro, seccion, subseccion, eval) VALUES ('$id_alumno', '$mes', '$ano','$param', '$numparam[$k]', '$seccion[$k]', '$subseccion[$k]', '$evaluacion[$k]')") or die(mysql_error());
		};
	...
	if ($pagina==0){
	//Es principal
	echo "document.location=('parametro_7meses.php?param=".$param."&nombre_completo=".$nombre_completo."&nivel=".$nivel."&grupo=".$grupo."&id_alumno=".$id_alumno."');";
	} else {
	//Es borrador
	echo "document.location=('borrador_parametro_7meses.php?param=".$param."&nombre_completo=".$nombre_completo."&nivel=".$nivel."&grupo=".$grupo."&id_alumno=".$id_alumno."&mes=".$mes."&ano=".$ano."');";	
	}	
	echo "</script>";
}
else
